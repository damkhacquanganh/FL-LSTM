...
def run_federated_training_pytorch(global_model, client_data, num_rounds=100, local_epochs=5, device='cpu', algo='fedavg', mu=0.01, eval_fn=None):
    """
    ...
    Args:
        ...
        algo (str): Thuật toán FL ('fedavg' hoặc 'fedprox').
        mu (float): Tham số cho FedProx.
        eval_fn (function): Hàm đánh giá hội tụ.
    ...
    for round_num in range(1, num_rounds + 1):
        algo_name = "FedAvg" if algo == 'fedavg' else f"FedProx (mu={mu})"
        print(f"\n[FL Round {round_num}/{num_rounds} - {algo_name}] Bắt đầu huấn luyện trên {num_clients} clients...")
        ...
        for client_id, (X_train, y_train) in enumerate(client_data):
            ...
            # Trích xuất global weights nếu dùng FedProx
            gw = None
            if algo == 'fedprox':
                gw = [param.data.clone() for param in global_model.parameters()]
        ...
        if eval_fn is not None:
            print(" Hoàn thành. → Đang đánh giá hội tụ...", end='', flush=True)
            acc = eval_fn(global_model)
